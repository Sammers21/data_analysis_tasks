...
destring Global_Sales User_Score, replace force
...
cluster singlelinkage zCritic_Score zUser_Count zGlobal_Sales zUser_Score, measure(L2) name(myclust)
cluster stop myclust,  rule(calinski) 
cluster dendrogram myclust, horizontal cutnumber(2)
cluster kmeans zCritic_Score zUser_Count zGlobal_Sales zUser_Score, k(2) measure(L2) name(kclust)
sort kclust
by kclust: list  if _n<=5
save video_games_sales_with_clusters.dta, replace
exit